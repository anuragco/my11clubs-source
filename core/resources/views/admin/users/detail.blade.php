...
<div class="row">
    <div class="col-12">
        ...
        <div class="d-flex flex-wrap gap-3 mt-4">
            ...
            <div class="flex-fill">
                <button type="button" class="btn btn--success btn--shadow w-100 btn-lg" data-bs-toggle="modal" data-bs-target="#updateSalaryModal">
                    <i class="las la-money-bill-wave"></i> @lang('Update Salary')
                </button>
            </div>
            @if($user->kyc_data)
            <div class="flex-fill">
...
<!-- Update Salary Modal -->
<!-- Update Salary Modal -->
<div id="updateSalaryModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="updateSalaryModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <div class="form-group">
            <label for="salary_action">Action</label>
            <select name="act" id="salary_action" class="form-control" required>
                <!--<option value="set">Set Salary</option>-->
                <option value="add">@lang ('Release Salary')</option>
            </select>
        </div>
        <div class="form-group">
            <label for="amount">Salary Amount</label>
            <input type="number" name="amount" id="amount" class="form-control" placeholder="Enter salary amount" required>
        </div>
        <div class="form-group">
...
<a href="{{ route('admin.users.login',$user->id) }}" class="btn btn--primary box--shadow1 text--small"><i class="las la-sign-in-alt"></i>@lang('Login as User')</a>
@endpush
...
$(document).ready(function() {
    $('#updateSalaryForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route("admin.users.update.salary", $user->id) }}',
            data: formData,
            success: function(response) {
                // Debugging: Log server response to console
                console.log(response);

                if(response.success) {
                    alert('Salary updated successfully');
                    location.reload();  // Reload page to update the salary widget
                } else {
                    alert('Failed to update salary');
                }
            },
            error: function(xhr) {
                // Debugging: Log error details to console
