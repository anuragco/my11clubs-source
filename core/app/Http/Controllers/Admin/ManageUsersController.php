    ...
    public function detail($id)
{
    ...
    // Fetching user's salary
    $userSalary = $user->salary;

    return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries', 'userSalary'));
}
...
public function updateSalary(Request $request, $userId) {
    $request->validate([
        ...
        'act' => 'required|in:add,set',
        'remark' => 'required|string|max:255',
    ]);

    $user = User::findOrFail($userId);
    $amount = $request->amount;
    $trx = getTrx();  // Assuming getTrx() generates a unique transaction ID

    $transaction = new Transaction();

    try {
        if ($request->act == 'add') {
            // Add the amount to both salary and balance
            $user->salary += $amount;
            $user->balance += $amount;

            $transaction->trx_type = '+';
            $transaction->remark = 'salary_add';

            $notifyTemplate = 'SAL_ADD';
            $notify[] = ['success', 'Salary added successfully'];

        } else {
            // Set the salary and add the amount to the balance
            $user->balance += $amount - $user->salary;
            $user->salary = $amount;

            $transaction->trx_type = '+';
            $transaction->remark = 'salary_set';

            $notifyTemplate = 'SAL_SET';
