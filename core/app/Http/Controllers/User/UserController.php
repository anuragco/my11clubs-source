...
{
    \Log::info('Starting home function');
    ...
    $ptc = PtcView::where('user_id', $user->id)->get(['view_date', 'amount']);
    \Log::info('PTC count: ' . $ptc->count());

    $totalincomeamountvalue = PtcView::where('user_id', $user->id)->sum('amount');
    \Log::info('Total income amount value: ' . $totalincomeamountvalue);

    $totaladsincome = $totalincomeamountvalue > 0 ? number_format($totalincomeamountvalue, 2) : '0.00';
    \Log::info('Total ads income: ' . $totaladsincome);

    $chart['click'] = $ptc->groupBy('view_date')->map(function ($item) {
        return collect($item)->count();
    })->sortDesc()->take(7)->toArray();
    \Log::info('Chart click: ' . json_encode($chart['click']));

    $chart['amount'] = $ptc->groupBy('view_date')->map(function ($item) {
        return collect($item)->sum('amount');
    })->sortDesc()->take(7)->toArray();
    \Log::info('Chart amount: ' . json_encode($chart['amount']));

    $totalcommitionalllevel = CommissionLog::where('to_id', $user->id)->sum('amount');
    \Log::info('Total commission all level (raw): ' . $totalcommitionalllevel);
    $totalcommitionalllevel = $totalcommitionalllevel > 0 ? $totalcommitionalllevel : 0;
    \Log::info('Total commission all level (after check): ' . $totalcommitionalllevel);

    $commissionCount = CommissionLog::where('to_id', $user->id)
        ->where('level', 1)
        ->sum('amount');
    \Log::info('Commission count level 1 (raw): ' . $commissionCount);
    $commissionCount = $commissionCount > 0 ? $commissionCount : 0;
    \Log::info('Commission count level 1 (after check): ' . $commissionCount);

    $commissionCountExcludeLevel1 = CommissionLog::where('to_id', $user->id)
        ->where('level', '>', 1)
        ->where('level', '<=', 10)
        ->sum('amount');
    \Log::info('Commission count exclude level 1 (raw): ' . $commissionCountExcludeLevel1);
    $commissionCountExcludeLevel1 = $commissionCountExcludeLevel1 > 0 ? $commissionCountExcludeLevel1 : 0;
    \Log::info('Commission count exclude level 1 (after check): ' . $commissionCountExcludeLevel1);

    $activeAdCount = Ptc::where('status', 1)->where('user_id', $user->id)->count();
    \Log::info('Active ad count: ' . $activeAdCount);

    $salary = $user->salary;
    \Log::info('Salary: ' . $salary);

    $totalEarnings = (float)$totalcommitionalllevel + (float)$salary + (float)$totalincomeamountvalue;
    \Log::info('Total earnings: ' . $totalEarnings);

    \Log::info('Final values', [
        'totalcommitionalllevel' => $totalcommitionalllevel,
        'commissionCount' => $commissionCount,
        'commissionCountExcludeLevel1' => $commissionCountExcludeLevel1,
        'totaladsincome' => $totaladsincome,
        'salary' => $salary,
        'totalEarnings' => $totalEarnings
    ]);

    return view('Template::user.dashboard', compact(
        'pageTitle', 'chart', 'user', 'commissionCount', 'activeAdCount',
        'commissionCountExcludeLevel1', 'totalcommitionalllevel', 'salary',
        'totalEarnings', 'totaladsincome'
    ));
}
